...
# 페이지 소스 코드 확인
페이지 소스(Page Source)는 우리가 요청을 보낼 때마다 웹 서버가 우리의 브라우저/클라이언트에 반환하는 사람이 읽을 수 있는 코드입니다.

반환되는 코드는 HTML(HyperText Markup Language), CSS(Cascading Style Sheets), JavaScript로 구성되어 있으며, 브라우저에 어떤 콘텐츠를 표시할지, 어떻게 보여줄지, 그리고 JavaScript를 통해 상호작용 요소를 추가할지를 알려줍니다.

우리 목적에서는 페이지 소스를 보는 것이 웹 애플리케이션에 대한 더 많은 정보를 발견하는 데 도움이 될 수 있습니다.

페이지 소스를 어떻게 보는가?

웹사이트를 보고 있는 동안 페이지를 우클릭하면 메뉴에서 “View Page Source(페이지 소스 보기)” 옵션을 볼 수 있습니다.

대부분의 브라우저는 URL 앞에 view-source: 를 붙이는 방법도 지원합니다. 예를 들어:

view-source:https://www.google.com/

브라우저 메뉴에서도 페이지 소스를 보는 옵션을 찾을 수 있습니다. 이 옵션은 경우에 따라 개발자 도구(Developer Tools)나 More Tools 같은 하위 메뉴 안에 있을 수 있습니다.

이제 페이지 소스를 살펴봅시다!

Acme IT Support 웹사이트의 홈 페이지 소스를 확인해보세요.
안타깝게도 여기서 볼 수 있는 모든 것을 설명하는 것은 이 룸의 범위를 훨씬 벗어나며, 이를 완전히 이해하려면 웹사이트 디자인/개발 과정을 따로 공부해야 합니다. 하지만 우리가 할 수 있는 것은 우리에게 중요한 정보 일부를 골라내는 것입니다.

페이지 상단에는 <!-- 로 시작하고 --> 로 끝나는 코드가 보일 것입니다. 이것은 주석(Comment)입니다.

주석은 웹사이트 개발자가 남겨놓은 메시지이며, 일반적으로 다른 프로그래머에게 코드 설명을 하거나 자기 자신을 위한 메모/알림 용도로 사용됩니다. 이러한 주석은 실제 웹페이지에는 표시되지 않습니다.

이 주석은 새로운 홈페이지가 개발되는 동안 현재 홈페이지가 임시라는 내용을 설명하고 있습니다. 주석 안의 웹페이지를 열어 첫 번째 플래그를 획득하세요.

HTML에서 다른 페이지로 이동하는 링크는 Anchor 태그( `<a>` 로 시작하는 HTML 요소 )로 작성되며, 이동할 링크는 href 속성에 저장됩니다.

예를 들어 31번째 줄에서 contact 페이지 링크를 볼 수 있습니다.

페이지 소스를 더 아래로 보면 "secr" 로 시작하는 페이지에 대한 숨겨진 링크가 있습니다. 이 링크를 열어 또 다른 플래그를 획득하세요.

실제 환경에서는 당연히 플래그를 얻지는 않겠지만, 기업이 회사/직원/고객 정보를 저장하기 위해 사용하는 비공개 영역을 발견할 수도 있습니다.

CSS, JavaScript, 이미지 같은 외부 파일은 HTML 코드 안에서 포함될 수 있습니다. 이 예시에서는 이러한 파일들이 모두 같은 디렉터리에 저장되어 있는 것을 볼 수 있습니다.

브라우저에서 해당 디렉터리를 열어보면 설정 오류가 존재합니다. 정상이라면 빈 페이지나 접근 권한이 없다는 403 Forbidden 오류 페이지가 표시되어야 합니다.

하지만 대신 디렉터리 목록 보기 기능이 활성화되어 있어 디렉터리 안의 모든 파일 목록이 표시됩니다.

때로는 이것이 문제가 아닐 수도 있고 디렉터리 안의 모든 파일이 공개되어도 안전할 수 있습니다. 하지만 경우에 따라 백업 파일, 소스 코드 또는 기타 기밀 정보가 저장되어 있을 수도 있습니다.

이 예시에서는 flag.txt 파일에서 플래그를 얻을 수 있습니다.

오늘날 많은 웹사이트는 처음부터 직접 만들어지지 않고 프레임워크(Framework)를 사용합니다.

프레임워크는 블로그, 사용자 관리, 폼 처리 등 웹사이트에 필요한 일반 기능들을 쉽게 포함할 수 있도록 미리 만들어진 코드 모음입니다. 이를 통해 개발자는 수시간 또는 수일의 개발 시간을 절약할 수 있습니다.

페이지 소스를 보면 프레임워크 사용 여부, 어떤 프레임워크인지, 심지어 어떤 버전인지에 대한 단서를 얻을 수 있는 경우가 많습니다.

프레임워크와 버전을 아는 것은 매우 강력한 정보가 될 수 있습니다. 해당 프레임워크에 공개된 취약점이 존재할 수 있고, 웹사이트가 최신 버전을 사용하지 않을 수도 있기 때문입니다.

페이지 하단에서는 사용 중인 프레임워크와 버전에 대한 주석, 그리고 프레임워크 웹사이트 링크를 찾을 수 있습니다.

프레임워크 웹사이트를 확인하면 현재 웹사이트가 실제로 오래된 버전을 사용 중이라는 것을 알 수 있습니다. 업데이트 공지를 읽고, 거기서 발견한 정보를 사용해 또 다른 플래그를 찾아보세요.
